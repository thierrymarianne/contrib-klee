...
check_bool() {
    ...
    local result
    result=$(to_bool "${!v}") || {
        echo "${v} has invalid boolean value: ${!v}"
        exit 1
    }
}
...
load_component() {
    ...
    source "${component_path}" || {
        echo "Error in file ${component_path}"
        return 1
    }
}
...
check_docker_os() {
    ...
    os_info=$(docker run --rm -v "${DIR}:/tmp" "${docker_base}" /tmp/build.sh --check-os) || {
        echo "Docker execution failed: $os_info"
        exit 1
    }
...
build_docker() {
    ...
    load_component "${component}" "v" || {
        echo "Loading component failed ${component}"
        return 1
    }
    ...
    for v in "${depending_artifact_components[@]}"; do
        [[ -z "${v}" ]] && continue
        { build_docker "${v}" 1; } || {
            echo "Not all compile dependencies are available: ${v}"
            return 1
        }
    done
    ...
    for v in "${depending_runtime_artifact_components[@]}"; do
        [[ -z "${v}" ]] && continue
        { build_docker "${v}" 1; } || {
            echo "Not all runtime dependencies are available: ${v}"
            return 1
        }
    done
    ...
    [[ -z $(docker images -q "${REPOSITORY}/${target_image}") ]] || {
        echo "Docker image exists: ${REPOSITORY}/${target_image}"
        return 0
    }
    ...
    echo docker build -t "${REPOSITORY}/${target_image}" -f "${temp_dir}"/Dockerfile "${docker_context}" || return 1
...
check_module() {
    ...
    (
        ...
        echo "Module $1 not valid"
        return 1
    }

    # Validate build instructions
    (
        validate_component "${module}" "p" || {
            echo "No build instructions found for ${module}"
            return 0
        }
        ...
        echo "Build instructions for ${module} are not valid"
        return 1
    }
...
install_component() {
    ...
    execution_action is_installed "${component}" && execution_action setup_artifact_variables "${component}" && validate_required_variables "${component}" "export_variables" && {
        echo "Already installed ${component}"
        return 0
    }

    # Install package if available
    execution_action install_binary_artifact "${component}" && execution_action setup_artifact_variables "${component}" && validate_required_variables "${component}" "export_variables" && {
        echo "Package installed ${component}"
        return 0
    }
...
main() {
    ...
    if [[ "${BUILD_DOCKER}" == "1" ]]; then
        ...
        for c in "${COMPONENTS[@]}"; do
            build_docker "$c" 0 || {
                echo "Building docker image ${c} failed"
                return 1
            }
        done
    else
